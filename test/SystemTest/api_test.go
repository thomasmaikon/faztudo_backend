...
func TestMain(m *testing.M) {
	...
	// initialize app
	app = App.NewApp().InitializeRoutes().InitializeDatabase(db).Router
	//Run tests
	code := m.Run()
...
func TestSimpleListPersonPaginatedServicePage(t *testing.T) {
	...
	apitest.New().
		...
		Expect(t).
		Status(http.StatusOK).
		Body(`{"services":[{"Id":2,"Name":"SimpleService","Description":"MockServiceCreated","Image":"SimplePathFromImage","Value":80.8,"PositiveEvaluations":null,"NegativeEvaluations":null}]}`).
		End()
